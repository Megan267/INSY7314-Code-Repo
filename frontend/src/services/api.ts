...
export async function getBooks() : Promise<Book[]> {
                            // fetching(http://localhost:300/api/books)
...
export async function getBook(id : string) : Promise<Book> {
                            // fetching(http://localhost:300/api/books/abc123)
    const response = await fetch(`${API_URL}/books/${id}`)
    return response.json();
}

//dont have to specify method above because default is GET
...
//this function will update section of an existing Book in the database using PATCH
